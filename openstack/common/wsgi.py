...
class AppFactory(BasePasteFactory):
    ...
    This requires tdafcommon.app_factory to be set to a callable which returns a
    WSGI app when invoked. The format of the name is <module>:<callable> e.g.

      [app:apiv1app]
      paste.app_factory = tdafcommon.common.wsgi:app_factory
      tdafcommon.app_factory = tdafcommon.api.cfn.v1:API

    The WSGI app constructor must accept a ConfigOpts object and a local config
    dict as its two arguments.
    """

    KEY = 'tdafcommon.app_factory'
...
class FilterFactory(AppFactory):
    ...
    This requires tdafcommon.filter_factory to be set to a callable which returns a
    WSGI filter when invoked. The format is <module>:<callable> e.g.

      [filter:cache]
      paste.filter_factory = tdafcommon.common.wsgi:filter_factory
      tdafcommon.filter_factory = tdafcommon.api.middleware.cache:CacheFilter

    The WSGI filter constructor must accept a WSGI app, a ConfigOpts object and
    a local config dict as its three arguments.
    """

    KEY = 'tdafcommon.filter_factory'
